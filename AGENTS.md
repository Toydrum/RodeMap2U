...
7. **Privacy: local-first, zero backend.** No accounts, no cloud, no analytics. Do not add network calls. Notifications exist ONLY as the opt-in **whispers** (`core/accompaniment.service.ts`): orientation QUESTIONS ("¿dónde sientes que estás?") that land on the check-in — **they may ask how you are; they may never tell you what to do**. No task names, no work, no counts of missed anything, silent (no sound/vibration), waking hours only (9–21), never during a session, never within 3h of a check-in, varying interval (fixed pings go blind). Off by default; the toggle is the permission gesture. Technical floor: they live only while the app is open somewhere (no push backend) — never promise more.
